...
class HyperspectralImageDataset(BaseImageDataset):
    def __init__(
        self, dir, dataroot, loadRGB, transforms_=None, unaligned=False, mode="train"
    ):
        ...
        if (
            mode == "train"
        ):  # capturing items from corresponding directory depending on training mode
            self.files_A = sorted(
                glob.glob(os.path.join(dir, dataroot, "trainA") + "/*.*")
            )  # tiff format, can be interchangeable
            self.files_B = sorted(
                glob.glob(os.path.join(dir, dataroot, "trainB") + "/*.*")
            )
        elif mode == "test":
            self.files_A = sorted(
                glob.glob(os.path.join(dir, dataroot, "testA") + "/*.*")
            )
            self.files_B = sorted(
                glob.glob(os.path.join(dir, dataroot, "testB") + "/*.*")
            )
    ...
    def __getitem__(self, index):
        ...
        )  # processing and transforming data to tensor
        ...
        if self.unaligned:
            item_B = self.files_B[
                random.randint(0, len(self.files_B) - 1)
            ]  # capturing random pair from B file directory if unaligned
        else:
            item_B = self.files_B[
                index % len(self.files_B)
            ]  # else, picking match from directory

        item_B = self.transform(
            proc.prepare_data(item_B)
        )  # processing and transforming data to tensor
