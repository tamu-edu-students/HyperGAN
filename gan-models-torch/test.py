...
class Test():
    ...
    def __init__(self) -> None:
        ...
        self.iters = 0
        py.mkdir(self.model.output_dir)
    ...
    def test_sequence(self):
        ...
        for i, batch in tqdm.tqdm(enumerate(self.dataloader), desc='Test Loop', total=len(self.dataloader.dataset)):
            
            self.model.set_input(batch)
            self.iters += 1
            self.model.forward()
            visuals = self.model.get_visuals(self.iters)
            visual_bytes = BytesIO()
            visuals.save(visual_bytes, format='JPEG')

            self.image_queue.put(visual_bytes.getvalue())
...
if __name__ == '__main__':
    ...
    web_output = WebOutput(image_generator)
